...
function ViewOrderModal(props) {
  ...
  useEffect(() => {
    if (props.data) {
      ...
      if (btnSync.current)
        btnSync.current.disabled =
          FINISHED_STATUS.indexOf(props.data.status) !== -1;
    }
  }, [props.data]);
  ...
  function onCancelClick(event) {
    ...
    cancelOrder(order.symbol, order.orderId, token)
      .then((result) => {
        ...
        if (props.onCancel)
          props.onCancel({ target: { id: "order", value: order.oderId } });
        ...
        <div className="modal-content">
          ...
          <form>
            ...
            <div className="modal-footer">
              {error ? (
                <div className="alert alert-danger mt-1 col-7 py-1">
                  {error}
                </div>
              ) : (
                <React.Fragment></React.Fragment>
