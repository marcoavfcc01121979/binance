...
function AutomationModal(props) {
  ...
  useEffect(() => {
    ...
    if (automation.symbol.startsWith("*"))
      ...
      const token = localStorage.getItem("token");
      getSymbol(automation.symbol, token).then((symbolObj) =>
        setSymbol(symbolObj)
      );
    }
  }, [automation.symbol]);
      ...
      .then((indexes) => {
        ...
        if (isWildcard) {
          ...
          const baseWallet = indexes.find(
            (ix) => ix.variable === "WALLET" && symbol.base === ix.symbol
          );
          ...
          const quoteWallet = indexes.find(
            (ix) =>
              ix.variable === "WALLET" &&
              symbol.quote === ix.symbol.replace("*", "")
          );
        ...
        <div className="modal-content">
          <div className="modal-header">
            <p className="modal-title" id="modalTitleNotify">
              {props.data.id ? "Edit " : "New "}
              {props.data.schedule ? "Scheduled " : ""}Automation
            </p>
            <button
              ref={btnClose}
              type="button"
