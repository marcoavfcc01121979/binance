...
async function startAutomation(req, res, next) {
  ...
  //     }
  //   } else beholder.updateBrain(automation.get({ plain: true }));

  await automation.save();

  if (automation.logs)
    logger("A:" + automation.id, `Automation ${automation.name} has started!`);
...
async function stopAutomation(req, res, next) {
  ...
  //   if (automation.schedule) agenda.cancelSchedule(automation.id);
  //   else beholder.deleteBrain(automation.get({ plain: true }));

  automation.isActive = false;
  await automation.save();

  if (automation.logs)
    logger("A:" + automation.id, `Automation ${automation.name} has stopped!`);
...
async function insertAutomation(req, res, next) {
  ...
  if (!newAutomation.actions || newAutomation.actions.length < 1)
    return res
      .status(400)
      .json("You need to have at least one action per automation!");

  //   const isGrid =
  //     newAutomation.actions[0].type === actionsRepository.actionTypes.GRID;
  //   if (isGrid) {
  //     if (!quantity || !levels)
  ...
  //       return res
  //         .status(409)
  //         .json(`A grid for ${newAutomation.symbol} already exists!`);
  //   } else {
  //     const exists = await automationsRepository.automationExists(
  //       newAutomation.name
  //     );
  //     if (exists)
  //       return res
  //         .status(409)
  //         .json(`The automation ${newAutomation.name} already exists!`);
  //   }
  ...
  //     //inserting grids
  //     if (isGrid)
  //       grids = await beholder.generateGrids(
  //         savedAutomation,
  //         levels,
  //         quantity,
  //         transaction
  ...
  //         return res.status(422).json(err.message);
  //       }
  //     } else beholder.updateBrain(savedAutomation.get({ plain: true }));
  //   }
...
async function updateAutomation(req, res, next) {
  ...
  //     }
  //   } else {
  //     if (updatedAutomation.schedule) agenda.cancelSchedule(updatedAutomation.id);
  //     else beholder.deleteBrain(currentAutomation);
  //   }
...
async function deleteAutomation(req, res, next) {
  ...
  if (currentAutomation.isActive) {
    // if (currentAutomation.schedule) agenda.cancelSchedule(currentAutomation.id);
    // else beholder.deleteBrain(currentAutomation);
  }
  ...
  //     if (
  //       currentAutomation.actions[0].type === actionsRepository.actionTypes.GRID
  //     ) {
  //       await gridsRepository.deleteGrids(id, transaction);
  //       await orderTemplatesRepository.deleteOrderTemplatesByGridName(
