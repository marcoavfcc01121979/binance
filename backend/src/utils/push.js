...
function addToCache(data) {
  if (cache && cache.length) cache.push(data);
  else cache = [data];
}
...
async function send(
  ...
  if (response.data.errors || response.data.data.status === "error") {
    ...
    throw new Error(
      `There was an error sending push notifications to ${settings.email}. The push token was cleaned!`
    );
  }
}
